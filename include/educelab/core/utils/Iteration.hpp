...
private:
    /**
     * @brief Iterator which increments from a starting value to a maximum value
    ...
    public:
        ...
        auto operator*() const -> reference { return value_; }

        /** Equality comparison */
        auto operator==(const CountingIterator& other) const -> bool
        {
        ...
        /** Increment operator */
        auto operator++() -> CountingIterator&
        {
...
public:
    ...
    auto begin() const -> iterator { return iterator{start_, end_, step_}; }
    /** Return the end of the range */
    auto end() const -> iterator { return iterator{end_, end_, step_}; }
    ...
    {
        return const_iterator{end_, end_, step_};
    }
...
template <typename T>
auto operator<<(std::ostream& os, const RangeIterable<T>& it) -> std::ostream&
{
    for (const auto& v : it) {
        if (v != *it.begin()) {
            os << " ";
...
/**
 * @brief Convenience method for constructing iterable ranges. Starting value is
 * 0.
 *
 ...
 */
template <typename T>
auto range(T stop) -> RangeIterable<T>
{
    return RangeIterable<T>(0, stop, 1);
}

/**
 * @brief Convenience method for constructing iterable ranges. Step value is 1.
 *
...
auto range(T0 start, T1 stop) -> RangeIterable<T0>
{
    return RangeIterable<T0>(start, stop, 1);
}

/**
 * @brief Convenience method for constructing iterable ranges.
 *
 * @param start starting value
 * @param stop ending value (non-inclusive)
...
auto range(T0 start, T1 stop, T2 step) -> RangeIterable<T0>
{
...
{
private:
    /** @brief Iterator which increments a counter over 2D values */
    template <typename ItT>
    ...
    public:
        ...
        /** Get the current value */
        auto operator*() const -> reference { return {v_, u_}; }

        /** Equality comparison */
        auto operator==(const Counting2DIterator& other) const -> bool
        {
        ...
        /** Increment operator */
        auto operator++() -> Counting2DIterator&
        {
            if (u_ < uMax_) {
...
public:
    ...
    {
        return iterator{vEnd_, vEnd_, uEnd_, uEnd_, step_};
    }
    /** Return the const start of the range */
    auto cbegin() const -> const_iterator
    {
...
/**
 * @brief Convenience method for constructing iterable 2D ranges. Starting value
 * is 0.
 *
 ...
 */
template <typename T0, typename T1>
auto range2D(T0 vStop, T1 uStop) -> Range2DIterable<T0>
{
    return Range2DIterable<T0>(0, vStop, 0, uStop, 1);
}

/**
 * @brief Convenience method for constructing iterable 2D ranges. Step value
 * is 1.
 *
...
auto range2D(T0 vStart, T1 vStop, T2 uStart, T3 uStop) -> Range2DIterable<T0>
{
    return Range2DIterable<T0>(vStart, vStop, uStart, uStop, 1);
}

/**
 * @brief Convenience method for constructing iterable 2D ranges.
 *
 * @param vStart Starting value for outer loop index
 * @param vStop Upper limit for outer loop
 * @param uStart Starting value for inner loop index
...
private:
    /**
     * @brief Enumerate iterator class. Tracks the iterator and index of the
     * item
     */
    template <class T>
    ...
    public:
        ...
        auto operator*() const -> value_type { return {idx_, *it_}; }
...
public:
    ...
    auto begin() -> iterator { return iterator{0, std::begin(container_)}; }

    /** Get the end-valued EnumerateIterable::iterator */
    auto end() -> iterator { return iterator{0, std::end(container_)}; }
